...
Màn hình **Settings** (trang chủ → **Configuration → Settings**) kiểm soát cách AI viết reply cho bạn — và các lựa chọn này áp dụng ở mọi nơi: X, Farcaster, và Binance Square. Các hành vi theo lượt chạy như độ trễ reply và bộ lọc điểm nằm bên trong từng chế độ tự động, không phải ở đây — chúng được nói đến trong mục [Bộ lọc điểm InfoFi](#b-l-c-i-m-infofi) bên dưới và trong [Hướng dẫn sử dụng](/docs?doc=usage-guide&lang=vi).
...
> **Binance Square** không dùng bộ lọc điểm InfoFi — các chế độ của nó có tùy chọn riêng ngay trong bảng điều khiển. Các cài đặt AI ở trên vẫn áp dụng cho Binance Square.
